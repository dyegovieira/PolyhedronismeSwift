...
internal struct DefaultPolyhedronCanonicalizer: PolyhedronCanonicalizer {
    ...
    public func adjust(_ polyhedron: Polyhedron, iterations: Int) async throws -> Polyhedron {
        let nIter = iterations
        let dpolyModel = try await dualOperator.apply(to: PolyhedronModel(polyhedron))
        var dpoly = Polyhedron(dpolyModel, recipe: polyhedron.recipe)

        var polyMut = Polyhedron(
            vertices: polyhedron.vertices,
            faces: polyhedron.faces,
            name: polyhedron.name,
            faceClasses: polyhedron.faceClasses,
            recipe: polyhedron.recipe
        )
        for _ in 0..<nIter {
            try Task.checkCancellation()
            let dStage = try await pipelineActor.reciprocalC(vertices: ContiguousArray(polyMut.vertices))
            logTelemetry(dStage.telemetry, polyName: polyhedron.name)
            dpoly.vertices = dStage.asArray()

            try Task.checkCancellation()
            let polyStage = try await pipelineActor.reciprocalC(vertices: ContiguousArray(dpoly.vertices))
            logTelemetry(polyStage.telemetry, polyName: polyhedron.name)
            polyMut.vertices = polyStage.asArray()
        }

        return Polyhedron(
            vertices: polyMut.vertices,
            faces: polyMut.faces,
            name: polyMut.name,
            faceClasses: polyMut.faceClasses,
            recipe: polyMut.recipe
        )
    }

    public func canonicalize(_ polyhedron: Polyhedron, iterations: Int) async throws -> Polyhedron {
        let nIter = iterations
        let dpolyModel = try await dualOperator.apply(to: PolyhedronModel(polyhedron))
        var dpoly = Polyhedron(dpolyModel, recipe: polyhedron.recipe)
        ...
        for _ in 0..<nIter {
            try Task.checkCancellation()
            let dStage = try await pipelineActor.reciprocalN(
                vertices: ContiguousArray(polyMut.vertices),
                faces: polyMut.faces
            )
            logTelemetry(dStage.telemetry, polyName: polyhedron.name)
            dpoly.vertices = dStage.asArray()
            ...
            try Task.checkCancellation()
            let polyStage = try await pipelineActor.reciprocalN(
                vertices: ContiguousArray(dpoly.vertices),
                faces: dpoly.faces
            )
            logTelemetry(polyStage.telemetry, polyName: polyhedron.name)
            polyMut.vertices = polyStage.asArray()
