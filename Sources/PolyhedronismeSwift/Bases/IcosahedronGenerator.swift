//
// PolyhedronismeSwift
// IcosahedronGenerator.swift
//
// Icosahedron base polyhedron generator for Conway notation
//
// Created by Dyego Vieira de Paula on 2025-11-04
// Built with AI-assisted development via Cursor IDE
//
import Foundation

internal struct IcosahedronGenerator: BasePolyhedronGenerator {
    public let identifier: String = "I"
    
    public init() {}
    
    public func generate() async throws -> PolyhedronModel {
        return PolyhedronModel(
            vertices: [
                [0, 0, 1.176],
                [1.051, 0, 0.526],
                [0.324, 1.0, 0.525],
                [-0.851, 0.618, 0.526],
                [-0.851, -0.618, 0.526],
                [0.325, -1.0, 0.526],
                [0.851, 0.618, -0.526],
                [0.851, -0.618, -0.526],
                [-0.325, 1.0, -0.526],
                [-1.051, 0, -0.526],
                [-0.325, -1.0, -0.526],
                [0, 0, -1.176]
            ],
            faces: [
                [0, 1, 2], [0, 2, 3], [0, 3, 4], [0, 4, 5],
                [0, 5, 1], [1, 5, 7], [1, 7, 6], [1, 6, 2],
                [2, 6, 8], [2, 8, 3], [3, 8, 9], [3, 9, 4],
                [4, 9, 10], [4, 10, 5], [5, 10, 7], [6, 7, 11],
                [6, 11, 8], [7, 10, 11], [8, 11, 9], [9, 11, 10]
            ],
            name: "I",
            faceClasses: []
        )
    }
}

