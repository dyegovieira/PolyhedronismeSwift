...
internal struct PyramidGenerator: ParameterizedBasePolyhedronGenerator {
    ...
    public func generate(parameters: PyramidParameters) async throws -> PolyhedronModel {
        let poly = try await buildPyramid(parameters.n)
    ...
    private func buildPyramid(_ n: Int) async throws -> Polyhedron {
        ...
        return try await canonicalizer.canonicalize(poly, iterations: 3)
    }
}
